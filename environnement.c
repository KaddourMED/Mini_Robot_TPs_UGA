...
resultat_deplacement avancer_envt(Environnement *envt) {
  ...
  if ((x < 0) || (x >= largeur(&envt->t)) || (y < 0) ||
    ...
    etat = recuperer_etat_obs(envt->obs);

    if(etat != ETAT_MESURE) etat = transition(etat, PAS_MESURE); 
    envt->obs.etat = etat;

    switch (case_devant) {
      case LIBRE:
        avancer(&(envt->r));
        etat = transition(etat, AVANCER); 
        envt->obs.etat = etat;
        return OK_DEPL;
      case EAU:
        return PLOUF;
...
int mesure_envt(Environnement *envt, int d) {
  ...
  Etats etat; //etat de l'observateur
  ...
  switch (d) {
    ...
    case 1: // devant
      mx = x + dx;
      my = y + dy;
      etat = transition(etat, MESURE_DEVANT);
      break;
    case 2: // devant droite
      mx = x + dx - dy;
      my = y + dy + dx;
      etat = transition(etat, MESURE_DEVANT_DROIT);
      break;
    case 3: // droite
      mx = x - dy;
      my = y + dx;
      etat = transition(etat, MESURE_DROIT);
      break;
    case 4: // derrière droite
      mx = x - dx - dy;
      my = y - dy + dx;
      etat = transition(etat, MESURE_DERRIERE_DROIT);
      break;
    case 5: // derrière
      mx = x - dx;
      my = y - dy;
      etat = transition(etat, MESURE_DERRIERE);
      break;
    case 6: // derrière gauche
      mx = x - dx + dy;
      my = y - dy - dx;
      etat = transition(etat, MESURE_DERRIERE_GAUCHE);
      break;
    case 7: // gauche
      mx = x + dy;
      my = y - dx;
      etat = transition(etat, MESURE_GAUCHE);
      break;
    case 8: // devant gauche
      mx = x + dx + dy;
      my = y + dy - dx;
      etat = transition(etat, MESURE_DEVANT_GAUCHE);
      break;
    default:
      return 3; 
  }
